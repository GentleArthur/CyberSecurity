...
def attack(source, command, save, command_out):
    ...
    for step in range(steps):
        ...
        current_L2 = nn.MSELoss(adv_images, input)
        L2_loss = current_L2.sum()

        logits = getLogits(adv_images)
        logits2 = logits
        logits = logits.reshape(-1, 32)
        logits_softmax = torch.softmax(logits, dim=-1)
        ...
        res = processor.batch_decode(torch.argmax(logits2, dim=-1))
        print('Transcription:', res[0])

        if res[0] == command_out.upper():
            print('succeed')
            torchaudio.save(save, adv_images.detach().cpu().reshape(1, -1), 16000)
            break
...
if __name__ == '__main__':
    ...
    parser.add_argument('--command_out', dest='command_out', type=str, help='target command', default=text_out[0])
    args = parser.parse_args()

    y_torch, sr_tar = torchaudio.load(args.originalFlie, channels_first=False)
    y_torch = y_torch.squeeze(dim=1)
    y_torch.requires_grad = True

    attack(source=y_torch, command=args.command, save=args.save, command_out=args.command_out)
